...
public partial class Secure_Edit_Banner : System.Web.UI.Page
{
    ...
    {
        DataSet ds = obj.GetBannersById(Convert.ToInt32(Request.QueryString["ID"]));
    ...
    protected void imgAdd_Click(object sender, ImageClickEventArgs e)
    {
        if (PhotoImage.HasFile)
        {
            ...
            if (theSQL == "Success")
            {
                Response.Redirect("Add_Banner.aspx");
            }
        }
        else
        {
    ...
    protected string UploadImage(int targetSize, int Width, int Height)
    {
        ...
        try
        {
            HttpPostedFile myFile = PhotoImage.PostedFile;
            int nFileLength = myFile.ContentLength;

            string extension = Utility.GetPictureExtension(PhotoImage.FileName).ToUpper();
